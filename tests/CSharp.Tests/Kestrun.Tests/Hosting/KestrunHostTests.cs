...
{
    [Fact]
    public void LogEnableConfigurationCalled_LogsDebug()
    {
        ...
        host.LogEnableConfigurationCalled();
        // No exception means logging succeeded
    }

    [Fact]
    public void LogConfigurationAlreadyApplied_LogsDebug()
    {
        var logger = new LoggerConfiguration().MinimumLevel.Debug().WriteTo.Console().CreateLogger();
        var host = new KestrunHost("TestApp", logger);
        host.LogConfigurationAlreadyApplied();
        // No exception means logging succeeded
    }

    [Fact]
    public void CreateAndValidateRunspacePool_ReturnsPoolOrThrows()
    {
        var logger = new LoggerConfiguration().CreateLogger();
        var host = new KestrunHost("TestApp", logger);
        var pool = host.CreateAndValidateRunspacePool(2, null, null);
        Assert.NotNull(pool);
    }

    [Fact]
    public void ConfigureKestrelServer_DoesNotThrow()
    {
        var logger = new LoggerConfiguration().CreateLogger();
        var host = new KestrunHost("TestApp", logger);
        host.ConfigureKestrelServer();
        // No exception means configuration succeeded
    }

    [Fact]
    public void ConfigureNamedPipesIfNeeded_DoesNotThrow()
    {
        var logger = new LoggerConfiguration().CreateLogger();
        var host = new KestrunHost("TestApp", logger);
        // Should not throw even if NamedPipeOptions is null
        host.ConfigureNamedPipesIfNeeded();
    }

    [Fact]
    public void ConfigureKestrelListenersAndHttps_DoesNotThrow()
    {
        var logger = new LoggerConfiguration().CreateLogger();
        var host = new KestrunHost("TestApp", logger);
        host.ConfigureKestrelListenersAndHttps();
        // No exception means configuration succeeded
    }

    [Fact]
    public void LogConfiguredEndpoints_LogsEndpointsOrWarning()
    {
        var logger = new LoggerConfiguration().CreateLogger();
        var host = new KestrunHost("TestApp", logger);
        host.ConfigureKestrelServer();
        host.ConfigureKestrelListenersAndHttps();
        var app = host.Build();
        host.LogConfiguredEndpoints(app);
        // No exception means logging succeeded
    }

    [Fact]
    public void HandleConfigurationException_ThrowsInvalidOperationException()
    {
        var logger = new LoggerConfiguration().CreateLogger();
        var host = new KestrunHost("TestApp", logger);
        var ex = new Exception("Test error");
        var thrown = Assert.Throws<InvalidOperationException>(() => host.HandleConfigurationException(ex));
        Assert.Contains("Failed to apply configuration", thrown.Message);
    }
    [Fact]
