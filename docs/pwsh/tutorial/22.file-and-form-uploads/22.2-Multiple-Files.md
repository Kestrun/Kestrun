...
- Accessing files: `Add-KrFormRoute` injects a `$FormContext` variable. Read files from `$FormContext.Payload.Files`.
- Multiple values: a single field name can map to multiple files (e.g. `$payload.Files['files']`).
