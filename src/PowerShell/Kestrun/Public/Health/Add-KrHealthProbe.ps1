...
function Add-KrHealthProbe {
    ...
    process {
        ...
        try {
            ...
            Write-KrLog -Level Information -Message "Health probe '{0}' registered." -Properties $Name
            if ($PassThru.IsPresent) {
                return $hostResult
            }
        } catch {
            Write-KrLog -Level Error -Message "Failed to register health probe '{0}'." -Properties $Name -ErrorRecord $_
            throw
        }
    }
}
