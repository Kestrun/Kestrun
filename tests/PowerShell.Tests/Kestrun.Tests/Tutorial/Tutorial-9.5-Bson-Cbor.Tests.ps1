...
Describe 'Example 9.5-Bson-Cbor' {
    ...
    It 'BSON route returns expected content' {
        $bson = Invoke-TestRequest -Uri "$($script:instance.Url)/bson" -UseBasicParsing -TimeoutSec 8
    ...
    It 'CBOR route returns expected content' {
        $cbor = Invoke-TestRequest -Uri "$($script:instance.Url)/cbor" -UseBasicParsing -TimeoutSec 8
    ...
    It 'Plain JSON route returns expected content' {
        $plain = Invoke-TestRequest -Uri "$($script:instance.Url)/plain" -UseBasicParsing -TimeoutSec 8
