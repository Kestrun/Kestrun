...
- `examples/PowerShell/BikeRentalShop/README.md`
- `examples/PowerShell/BikeRentalShop/Synchronized/Service.ps1`
- `examples/PowerShell/BikeRentalShop/Synchronized/Private/State.ps1`
- `examples/PowerShell/BikeRentalShop/Synchronized/Private/Routes.ps1`
...
1. Entry point: Start `examples/PowerShell/BikeRentalShop/Synchronized/Service.ps1` directly from the repository checkout.
2. Runtime folders: The service prepares `data/`, `logs/`, and `data/certs/` so state, logs, and the HTTPS certificate survive restarts.
...
- `examples/PowerShell/BikeRentalShop/Synchronized/Service.ps1` is the package-ready backend entry point for the tutorial.
- `examples/PowerShell/BikeRentalShop/Synchronized/Private/State.ps1` owns storage initialization, persistence, and certificate helpers.
- `examples/PowerShell/BikeRentalShop/Synchronized/Private/Routes.ps1` maps the customer and staff APIs and keeps the route logic separate from host setup.
- `examples/PowerShell/BikeRentalShop/Concurrent/Service.ps1` keeps the same API contract but swaps the in-memory model to concurrent dictionaries.
- `examples/PowerShell/BikeRentalShop/Web/Service.ps1` adds a separate Razor Pages client that talks to the backend over HTTP.
...
| State resets unexpectedly | The sample is running from a different folder or its `data/` directory was cleared | Run the sample from the repository and check the `examples/PowerShell/BikeRentalShop/*/data/` folder |
