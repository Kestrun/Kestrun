...
2. A client certificate trusted by the server
...
$ca = New-KrSelfSignedCertificate -DnsNames 'Test CA' -CertificateAuthority

# Create a server certificate
$server = New-KrSelfSignedCertificate -DnsNames 'localhost' -SigningCertificate $ca

# Create a client certificate
$client = New-KrSelfSignedCertificate -DnsNames 'test-client' -SigningCertificate $ca -ClientAuth

# Export client certificate for testing
Export-KrCertificate -Certificate $client -FilePath 'client.pfx' `
    -IncludePrivateKey -Password (ConvertTo-SecureString -String 'test' -AsPlainText -Force)
```
