...
server.AddMapRoute("/", HttpVerb.Get, async (ctx) =>
{
    var html = """
    <!DOCTYPE html>
    <html>
    ...
    <body>
        ...
        <script>
            ...
            connection.on("ReceiveEcho", (message) => {
                addMessage(`<strong>Echo:</strong> ${message}`, "log-entry");
            });
    ...
    await ctx.Response.WriteHtmlResponseAsync(html, 200);
});

// Route to broadcast logs via C#
server.AddMapRoute("/api/log/{level}", HttpVerb.Get, async (ctx) =>
{
    var level = ctx.Request.RouteValues["level"]?.ToString() ?? "Information";
    var broadcaster = ctx.RequestServices.GetService(typeof(IRealtimeBroadcaster)) as IRealtimeBroadcaster;
    
    if (broadcaster != null)
    {
        await broadcaster.BroadcastLogAsync(level, $"Test {level} message from C# at {DateTime.Now:HH:mm:ss}", ctx.RequestAborted);
        await ctx.Response.WriteTextResponseAsync($"Broadcasted {level} log message", 200);
    }
    else
    {
        await ctx.Response.WriteTextResponseAsync("IRealtimeBroadcaster not available", 500);
    }
});

// Route to broadcast custom events via C#
server.AddMapRoute("/api/event", HttpVerb.Get, async (ctx) =>
{
    var broadcaster = ctx.RequestServices.GetService(typeof(IRealtimeBroadcaster)) as IRealtimeBroadcaster;
    
    if (broadcaster != null)
    {
        await broadcaster.BroadcastEventAsync(
            "CustomEvent",
            new { Message = "Hello from C#", Timestamp = DateTime.Now },
            ctx.RequestAborted
        );
        await ctx.Response.WriteTextResponseAsync("Broadcasted custom event", 200);
    }
    else
    {
        await ctx.Response.WriteTextResponseAsync("IRealtimeBroadcaster not available", 500);
