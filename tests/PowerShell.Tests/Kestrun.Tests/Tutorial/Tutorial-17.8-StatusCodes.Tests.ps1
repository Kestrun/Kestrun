...
Describe 'Example 17.8 Common Status Codes' -Tag 'Tutorial', 'StatusCodes', 'OpenApi', 'Slow' {
    ...
    It 'GET /public returns 200' {
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/public" -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 200
        $resp.Content | Should -Be 'OK'
    }

    It 'GET /secure/hello returns 401 without credentials' {
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/hello" -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 401
    }

    It 'GET /secure/hello returns 401 with bad credentials' {
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/hello" -Headers @{ Authorization = $script:badAuth } -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 401
    }

    It 'GET /secure/hello returns 200 with valid credentials' {
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/hello" -Headers @{ Authorization = $script:adminAuth } -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 200
    ...
    It 'DELETE /secure/resource/{id} returns 403 when missing CanDelete policy' {
        $resp = Invoke-TestRequest -Method Delete -Uri "$($script:instance.Url)/secure/resource/1" -Headers @{ Authorization = $script:adminAuth } -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 403

        $resp2 = Invoke-TestRequest -Method Delete -Uri "$($script:instance.Url)/secure/resource/1" -Headers @{ Authorization = $script:userAuth } -SkipCertificateCheck -SkipHttpErrorCheck
        $resp2.StatusCode | Should -Be 403
    }

    It 'POST /json/echo returns 415 when Content-Type is not application/json' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echo" -Body 'hi' -ContentType 'text/plain' -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 415
    }

    It 'POST /json/echo returns 415 when there is no Content-Type header' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echo" -Body 'hi' -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 415
    }

    It 'POST /json/echo returns 400 when JSON is invalid' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echo" -Body '{"a":' -ContentType 'application/json' -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 400
    }

    It 'POST /json/echo returns 422 when JSON is valid but fails validation (named properties)' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echo" -Body '{"name":"","quantity":2}' -ContentType 'application/json' -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 422
    }

    It 'POST /json/echo returns 422 when JSON is valid but fails validation (quantity is 0)' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echo" -Body '{"name":"widget","quantity":0}' -ContentType 'application/json' -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 422
    }

    It 'POST /json/echo returns 422 when JSON is valid but fails validation (additional property not allowed by schema)' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echo" -Body '{"name":"widget","quantity":2,"notValid":true}' -ContentType 'application/json' -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 422
    }

    It 'POST /json/echo returns 201 when JSON is valid' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echo" -Body '{"name":"widget","quantity":2,"priority":"normal"}' -ContentType 'application/json' -SkipCertificateCheck -SkipHttpErrorCheck
    ...
    It 'POST /json/echoPlus returns 422 when JSON is valid but fails validation (named properties)' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echoPlus" -Body '{"name":"","quantity":2}' -ContentType 'application/json' -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 422
    }

    It 'POST /json/echoPlus returns 422 when JSON is valid but fails validation (quantity is 0)' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echoPlus" -Body '{"name":"widget","quantity":0}' -ContentType 'application/json' -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 422
    }

    It 'POST /json/echoPlus returns 201 when JSON is valid' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/json/echoPlus" -Body '{"name":"widget","quantity":2,"priority":"normal","notValid":false}' `
    ...
    It 'POST /only-get returns 405 (method not allowed)' {
        $resp = Invoke-TestRequest -Method Post -Uri "$($script:instance.Url)/only-get" -Body '' -ContentType 'text/plain' -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 405
    }

    It 'GET /does-not-exist returns 404' {
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/does-not-exist" -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 404
    }

    It 'No-content contract mismatch returns 500' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/test" -SkipCertificateCheck -SkipHttpErrorCheck
        $result.StatusCode | Should -Be 500
        $result.Content | Should -Match 'declared without content'
    }

    It 'OpenAPI lists the endpoints' {
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/openapi/v3.2/openapi.json" -SkipCertificateCheck -SkipHttpErrorCheck
        $resp.StatusCode | Should -Be 200
