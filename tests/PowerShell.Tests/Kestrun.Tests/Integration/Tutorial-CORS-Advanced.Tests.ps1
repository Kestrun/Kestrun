...
Describe 'CORS Advanced Features' -Tag 'Integration', 'CORS' {
    ...
    Context 'Policy Not Found Behavior' {
        ...
        BeforeAll {
            $script:instance = Start-ExampleScript -Scriptblock {
                ...
                New-KrServer -Name 'PolicyNotFoundServer'
                Add-KrEndpoint -Port $Port -IPAddress $IPAddress

                # Register a policy but reference wrong name on route
                New-KrCorsPolicyBuilder |
                    Set-KrCorsOrigin -Origins 'http://localhost:3000' |
                    Set-KrCorsMethod -Any |
        ...
        It 'Returns 500 when policy is not found' {
            { Invoke-WebRequest -Uri "$($script:instance.Url)/misconfig" `
                    -Method Get `
                    -Headers @{ 'Origin' = 'http://localhost:3000' } } |
                Should -Throw
        }
