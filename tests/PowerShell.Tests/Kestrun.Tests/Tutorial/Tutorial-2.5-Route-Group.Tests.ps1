...
Describe 'Example 2.5-Route-Group' -Tag 'Tutorial' {
    ...
    It 'Grouped parameter routes return expected content for Get' {
        # Path (GET)
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/input/demoPath" -UseBasicParsing -TimeoutSec 6 -Method Get
    ...
    It 'Grouped parameter routes return expected content for PATCH' {
        # Query (PATCH)
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/input?value=demoQuery" -UseBasicParsing -TimeoutSec 6 -Method Patch
        $resp.StatusCode | Should -Be 200
    ...
    It 'Grouped parameter routes return expected content for POST' {
        ...
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/input" -UseBasicParsing -TimeoutSec 6 -Method Post -Body $body -ContentType 'application/json'
        $resp.StatusCode | Should -Be 200
        $resp.Content | Should -Be "The Body Parameter 'value' was: demoBody"
    }
    It 'Grouped parameter routes return expected content for PUT' {
        # Header (PUT)
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/input" -UseBasicParsing -TimeoutSec 6 -Method Put -Headers @{ value = 'demoHeader' }
        $resp.StatusCode | Should -Be 200
    ...
    It 'Grouped parameter routes return expected content for DELETE' {
        # Cookie (DELETE)
        $resp = Invoke-TestRequest -Uri "$($script:instance.Url)/input" -UseBasicParsing -TimeoutSec 6 -Method Delete -Headers @{ Cookie = 'value=demoCookie' }
        $resp.StatusCode | Should -Be 200
