...
5. **Wire up inputs**: Reference pagination components via `[OpenApiParameterRef]` and accept the structured filter body via `[OpenApiRequestBody]` using the `ProductSearchRequest` schema.
...
8. **Finalize configuration and start**: Register documentation UIs ([Add-KrApiDocumentationRoute][Add-KrApiDocumentationRoute]) and the OpenAPI document route ([Add-KrOpenApiRoute][Add-KrOpenApiRoute], `DefaultVersion '3.2'`), commit staged changes with [Enable-KrConfiguration][Enable-KrConfiguration], then start the server with [Start-KrServer][Start-KrServer].
