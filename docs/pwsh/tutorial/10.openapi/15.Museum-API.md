...
- **Reusable scalar components**: `Date : OpenApiDate` defines a `string` with `format: date` once,
      then fields like `ticketDate` and `MuseumDailyHours.date` use `[Date]` so the OpenAPI uses `$ref`.

1. **Parameters**: Define reusable parameters using `[OpenApiParameterComponent]`, then reference them from routes using `[OpenApiParameterRef]`.
2. **Security**: Implement `MuseumPlaceholderAuth` (Basic Auth) using `Add-KrBasicAuthentication`.
3. **Routes**: Implement operations:
...
4. **Documentation**: Enable OpenAPI route and build the document.

## Simple scalar schema components

The Museum example uses Kestrun's OpenAPI scalar wrapper types (like `OpenApiDate`) to create reusable primitive schemas.
