...
Describe 'Example 8.x Authentication (Multiple-Schemes)' -Tag 'Tutorial', 'Auth', 'Slow' {
    ...
    It 'GET /secure/basic returns 401 without credentials' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/basic" -SkipCertificateCheck -SkipHttpErrorCheck
        $result.StatusCode | Should -Be 401
    }

    It 'GET /secure/basic returns 200 with Basic credentials' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/basic" -SkipCertificateCheck -SkipHttpErrorCheck -Headers @{ Authorization = $script:basic }
    ...
    It 'GET /secure/key returns 200 with API key' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/key" -SkipCertificateCheck -SkipHttpErrorCheck -Headers @{ 'X-Api-Key' = 'my-secret-api-key' }
        $result.StatusCode | Should -Be 200
    ...
    It 'GET /secure/key returns 200 with Basic credentials (fallback scheme)' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/key" -SkipCertificateCheck -SkipHttpErrorCheck -Headers @{ Authorization = $script:basic }
        $result.StatusCode | Should -Be 200
        $result.Content | Should -Be 'Key OK'
    }

    It 'GET /secure/token/new issues a JWT token with Basic credentials' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/token/new" -SkipCertificateCheck -SkipHttpErrorCheck -Headers @{ Authorization = $script:basic }
        $result.StatusCode | Should -Be 200
    ...
    It 'GET /secure/jwt returns 200 with Bearer token' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/jwt" -SkipCertificateCheck -SkipHttpErrorCheck -Headers @{ Authorization = "Bearer $($script:token)" }
        $result.StatusCode | Should -Be 200
