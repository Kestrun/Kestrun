...
Describe 'Example 8.3 Authentication (ApiKey)' -Tag 'Tutorial', 'Slow' {
    ...
    It 'key authentication Hello Simple mode' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/key/simple/hello" -SkipCertificateCheck -Headers $script:headers
    ...
    It 'key authentication Hello in powershell' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/key/ps/hello" -SkipCertificateCheck -Headers $script:headers
        $result.Content | Should -Be 'PS Key OK'
        $result.Headers.'Content-Type' | Should -Be 'text/plain; charset=utf-8'
    }
    It 'key authentication Hello in CSharp' {
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/secure/key/cs/hello" -SkipCertificateCheck -Headers $script:headers
