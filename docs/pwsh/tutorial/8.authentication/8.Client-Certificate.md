...
Client certificate authentication is a secure way to verify the identity of clients connecting to your server using
X.509 certificates. This is particularly useful in scenarios where you need mutual TLS (mTLS) authentication.
...
# Configure HTTPS to require client certificates
Set-KrServerHttpsOptions -ClientCertificateMode RequireCertificate

# Add HTTPS endpoint
Add-KrEndpoint -Port 5001 -X509Certificate $cert
...
**Note:** For production environments, client certificates should be issued by a trusted Certificate Authority (CA).
Self-signed certificates are only suitable for development and testing.

### Test with PowerShell

```powershell
# Import the client certificate (cross-platform compatible)
$clientCert = Import-KrCertificate -FilePath 'client.pfx' `
...
**Note:** Use `Import-KrCertificate` instead of `Import-PfxCertificate` for cross-platform compatibility.
The Kestrun cmdlet doesn't require a Windows certificate store and works on both Windows and Linux.
...
### TLS Handshake Fails (Self-signed Client Certificates)

If you are using self-signed client certificates for local development, the TLS handshake can fail before Kestrun authentication runs.
In that case, configure Kestrel's TLS-level client certificate validation callback.

> **Important:** This callback runs on Kestrel TLS threads (no PowerShell runspace). Use a pure .NET delegate or compile C#/VB.NET code.

#### Option 1: Built-in .NET callback (recommended for tutorials/dev)
...
#### Option 2: Compile C# code with Roslyn
...
{
        return true; // AllowCertificate: permit connections without a client certificate
}

return sslPolicyErrors == SslPolicyErrors.None
        || sslPolicyErrors == SslPolicyErrors.RemoteCertificateChainErrors; // allow self-signed/untrusted chain in dev
'@
...
# Or load the snippet from a file and infer language from extension (.cs/.csx/.vb):
Set-KrServerHttpsOptions `
    -ClientCertificateMode AllowCertificate `
