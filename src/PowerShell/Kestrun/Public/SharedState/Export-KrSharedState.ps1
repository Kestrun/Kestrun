...
.DESCRIPTION
    ...
    Access is synchronized through the Kestrun lock registry so that callers
    using the same lock key serialize shared state in a thread-safe manner
...
.PARAMETER LockKey
    The logical lock key used to synchronize access through
    Kestrun.Utilities.KestrunLockRegistry.
...
.EXAMPLE
    Export-KrSharedState -InputObject $state -LockKey 'sharedstate:cache'
#>
<#
.SYNOPSIS
    Exports a PowerShell object to a serialized XML representation.
