...
3. **Define shared state** using `Set-KrSharedState`:
    ...
    - `Set-KrSharedState -Name 'StartTime' -Value (Get-Date)` — server start time
...
`Update-KrSynchronizedCounter` internally uses a lock-free / synchronized
increment (C# atomic primitives) ensuring each request contributes exactly
...
|--------|------------------------|----------------------------|
| **Thread safety** | Must use thread-safe types | Built-in synchronization |
