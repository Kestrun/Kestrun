...
Describe 'Example 10.3 OpenAPI Component RequestBody' -Tag 'OpenApi', 'Tutorial', 'Slow' {
    ...
    It 'Create Product (POST)' {
        ...
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/products" -Method Post -Body $body -ContentType 'application/json' -SkipCertificateCheck -SkipHttpErrorCheck
    ...
    It 'Update Product (PUT)' {
        ...
        $result = Invoke-TestRequest -Uri "$($script:instance.Url)/products/1" -Method Put -Body $body -ContentType 'application/json' -SkipCertificateCheck -SkipHttpErrorCheck
