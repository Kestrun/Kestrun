...
function Add-KrHealthProcessProbe {
    ...
    process {
        ...
        try {
            ...
            Write-KrLog -Level Information -Message "Process health probe '{0}' registered." -Value $Name
            if ($PassThru.IsPresent) {
                return $hostResult
            }
        } catch {
            Write-KrLog -Level Error -Message "Failed to register process health probe '{0}'." -Value $Name -ErrorRecord $_
            throw
        }
    }
}
