...
public partial class OpenApiDocDescriptor
{
    ...
    private void BuildHeaders(Type t)
    {
        ...
        foreach (var headerAttribute in headerAttributes)
        {
            ...
            {
                Description = string.IsNullOrWhiteSpace(headerAttribute.Description) ? null : headerAttribute.Description,
                Required = headerAttribute.Required,
                Deprecated = headerAttribute.Deprecated,
                AllowEmptyValue = headerAttribute.AllowEmptyValue,
                Style = headerAttribute.Style.ToOpenApi(),
                Explode = headerAttribute.Explode,
                AllowReserved = headerAttribute.AllowReserved,
                Example = OpenApiJsonNodeFactory.FromObject(headerAttribute.Example)
            };

            if (!string.IsNullOrWhiteSpace(headerAttribute.SchemaRef))
            {
                header.Schema = new OpenApiSchemaReference(headerAttribute.SchemaRef);
            }
            else
            {
                var schemaType = headerAttribute.Type?.Trim().ToLowerInvariant() switch
                {
                ...
                {
                    Type = schemaType,
                    Format = headerAttribute.Format
                };
            }

            if (!string.IsNullOrWhiteSpace(headerAttribute.ExampleRef) || headerAttribute.Example is not null)
            {
                header.Examples ??= new Dictionary<string, IOpenApiExample>(StringComparer.Ordinal);

                if (!string.IsNullOrWhiteSpace(headerAttribute.ExampleRef))
                {
                    header.Examples["exampleRef"] = new OpenApiExampleReference(headerAttribute.ExampleRef);
                }

                if (headerAttribute.Example is not null)
                {
                    header.Examples["example"] = new OpenApiExample
                    {
                        Value = OpenApiJsonNodeFactory.FromObject(headerAttribute.Example)
                    };
                }
            }

            _ = Document.Components.Headers.TryAdd(headerAttribute.Key, header);
        }
    }
