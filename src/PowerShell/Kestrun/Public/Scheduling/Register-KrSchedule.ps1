<#
    ...
    .EXAMPLE
        Register-KrSchedule -Name Heartbeat -Cron '*/10 * * * * *' -ScriptBlock {
            Write-KrLog -Level Information -Message "💓 Heartbeat at {0:O}" -PropertyValues $([DateTimeOffset]::UtcNow)
        }
    ...
    .EXAMPLE
        Register-KrSchedule -Server $server -Name 'RunOnce' -Interval '00:01:00' -ScriptBlock {
            Write-KrLog -Level Information -Message "Running once at {0:O}" -PropertyValues $([DateTimeOffset]::UtcNow)
        } -RunImmediately
    ...
    process {
        ...
        try {
            ...
            if ($PassThru.IsPresent) {
                ...
                Write-KrLog -Level Information -Message "Schedule '{0}' registered successfully." -PropertyValues $Name

                # return the freshly-registered JobInfo
                return $sched.GetSnapshot() | Where-Object Name -EQ $Name
            } else {
                Write-KrLog -Level Information -Message "Schedule '{0}' registered successfully. Use -PassThru to return the job info." -PropertyValues $Name
            }
        } catch {
