...
function Set-KrServerLimit {
    ...
    process {
        ...
        if ($MaxRequestBodySize -gt 0) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting MaxRequestBodySize to {MaxRequestBodySize} bytes" -PropertyValues $MaxRequestBodySize
            $options.ServerLimits.MaxRequestBodySize = $MaxRequestBodySize
        }
        if ($MaxConcurrentConnections -gt 0) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting MaxConcurrentConnections to {MaxConcurrentConnections}" -PropertyValues $MaxConcurrentConnections
            $options.ServerLimits.MaxConcurrentConnections = $MaxConcurrentConnections
        }
        if ($MaxRequestHeaderCount -gt 0) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting MaxRequestHeaderCount to {MaxRequestHeaderCount}" -PropertyValues $MaxRequestHeaderCount
            $options.ServerLimits.MaxRequestHeaderCount = $MaxRequestHeaderCount
        }
        if ($KeepAliveTimeoutSeconds -gt 0) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting KeepAliveTimeout to {KeepAliveTimeoutSeconds} seconds" -PropertyValues $KeepAliveTimeoutSeconds
            $options.ServerLimits.KeepAliveTimeout = [TimeSpan]::FromSeconds($KeepAliveTimeoutSeconds)
        }
        if ($MaxRequestBufferSize -gt 0) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting MaxRequestBufferSize to {MaxRequestBufferSize} bytes" -PropertyValues $MaxRequestBufferSize
            $options.ServerLimits.MaxRequestBufferSize = $MaxRequestBufferSize
        }
        if ($MaxRequestHeadersTotalSize -gt 0) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting MaxRequestHeadersTotalSize to {MaxRequestHeadersTotalSize} bytes" -PropertyValues $MaxRequestHeadersTotalSize
            $options.ServerLimits.MaxRequestHeadersTotalSize = $MaxRequestHeadersTotalSize
        }
        if ($MaxRequestLineSize -gt 0) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting MaxRequestLineSize to {MaxRequestLineSize} bytes" -PropertyValues $MaxRequestLineSize
            $options.ServerLimits.MaxRequestLineSize = $MaxRequestLineSize
        }
        if ($MaxResponseBufferSize -gt 0) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting MaxResponseBufferSize to {MaxResponseBufferSize} bytes" -PropertyValues $MaxResponseBufferSize
            $options.ServerLimits.MaxResponseBufferSize = $MaxResponseBufferSize
        }
        if ($null -ne $MinRequestBodyDataRate) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting MinRequestBodyDataRate to {MinRequestBodyDataRate} bytes/second" -PropertyValues $MinRequestBodyDataRate
            $options.ServerLimits.MinRequestBodyDataRate = $MinRequestBodyDataRate
        }
        if ($null -ne $MinResponseDataRate) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting MinResponseDataRate to {MinResponseDataRate} bytes/second" -PropertyValues $MinResponseDataRate
            $options.ServerLimits.MinResponseDataRate = $MinResponseDataRate
        }
        if ($null -ne $RequestHeadersTimeout) {
            Write-KrLog -Logger $Server.HostLogger -Level Verbose -Message "Setting RequestHeadersTimeout to {RequestHeadersTimeout} seconds" -PropertyValues $RequestHeadersTimeout
