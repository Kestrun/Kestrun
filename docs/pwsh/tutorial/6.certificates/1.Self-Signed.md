...
3. Create a development certificate with `New-KrSelfSignedCertificate`:
    - Dns: `-DnsNames 'localhost','127.0.0.1','::1'` for localhost work.
    - Key: `-KeyType Rsa|Ecdsa` and `-KeyLength` (e.g., 2048 for RSA).
    - KU (optional): `-KeyUsage DigitalSignature,KeyEncipherment` for RSA TLS server certs, or `DigitalSignature` for ECDSA.
...
5. Confirm localhost coverage:
    - `DNS:localhost`
    - `IP:127.0.0.1`
    - `IP:::1`
6. Create server: `New-KrServer`.
7. Bind HTTPS listener: `Add-KrEndpoint -Port 5001 -IPAddress Loopback -X509Certificate $cert`.
    - Optionally keep HTTP on 5000 for plaintext testing.
    - You can restrict protocols, e.g., `-Protocols Http1`.
8. Apply config: `Enable-KrConfiguration`.
9. Map a minimal route (e.g., GET `/hello`) returning JSON so you can verify TLS termination.
10. Start the server: `Start-KrServer`.
11. Client notes:
