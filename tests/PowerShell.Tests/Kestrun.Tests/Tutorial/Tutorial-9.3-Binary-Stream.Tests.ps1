...
Describe 'Example 9.3-Binary-Stream' {
    BeforeAll { $script:instance = Start-ExampleScript -Name '9.3-Binary-Stream.ps1' }
    AfterAll {
        if ($script:instance) {
    ...
    It 'Binary and stream routes return data or 404 gracefully' {
        $logo = Invoke-TestRequest -Uri "$($script:instance.Url)/logo" -UseBasicParsing -TimeoutSec 8 -ErrorAction SilentlyContinue
        ...
        $stream = Invoke-TestRequest -Uri "$($script:instance.Url)/stream" -UseBasicParsing -TimeoutSec 8 -ErrorAction SilentlyContinue
