...
- **Contact & License**: Standard OpenAPI metadata for ownership and usage terms. Contact supports vendor extensions via `Add-KrOpenApiContact -Extensions` (keys normalized to `x-` when missing). For licenses, you can use either `-Url` or the OpenAPI 3.1/3.2 `-Identifier` (SPDX).
