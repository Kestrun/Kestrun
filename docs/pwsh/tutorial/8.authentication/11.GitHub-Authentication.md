...
File: [`pwsh/tutorial/examples/8.11-GitHubAuthentication.ps1`][8.11-GitHubAuthentication.ps1]

```powershell
{% include examples/pwsh/8.11-GitHubAuthentication.ps1 %}
```
...
4. Auth: Call `Add-KrGitHubAuthentication` – registers base, cookie, and policy schemes plus PKCE and scope defaults.
   Use `-CallbackPath` to match your OAuth App (for example `/signin-oauth`).
5. Configuration: Finalize middleware with `Enable-KrConfiguration`.
6. Routes: Public landing + protected `/github/*` group referencing `GitHub.Policy` for forwarding.
...
pwsh docs/_includes/examples/pwsh/8.11-GitHubAuthentication.ps1
```
...
- Three schemes: `GitHub`, `GitHub.Cookies`, `GitHub.Policy` (policy forwards challenge → GitHub, auth → cookie).
...
- The GitHub OAuth App callback must match `https://localhost:<port><CallbackPath>`.
  Default path is `/signin-github`.
  If your app uses `/signin-oauth`, run with `-CallbackPath '/signin-oauth'` and update the app.
...
[8.11-GitHubAuthentication.ps1]: /pwsh/tutorial/examples/8.11-GitHubAuthentication.ps1
