...
File: [`pwsh/tutorial/examples/10.23-OpenAPI-VariableMapping.ps1`][10.23-OpenAPI-VariableMapping.ps1]

```powershell
{% include examples/pwsh/10.23-OpenAPI-VariableMapping.ps1 %}
```
...
6. **Multi-segment paths**:
   - **OpenAPI**: Document the route as `/files/{+path}` (RFC6570 reserved expansion).
   - **Runtime**: Map an ASP.NET Core catch-all route `/files/{*path}` so requests like `/files/a/b/c.txt` match and `RouteValues['path']` contains slashes.
...
```powershell
pwsh .\docs\_includes\examples\pwsh\10.23-OpenAPI-VariableMapping.ps1
```
...
- ASP.NET Core uses catch-all parameters (`{*path}`) to capture multiple segments at runtime.
- Kestrun bridges the gap by mapping ASP.NET route values into RFC6570 variable assignments.

## Troubleshooting

- **`/files/...` returns 404**: Ensure the runtime route uses a catch-all (`/files/{*path}`) so ASP.NET Core matches multi-segment paths.
...
[10.23-OpenAPI-VariableMapping.ps1]: /pwsh/tutorial/examples/10.23-OpenAPI-VariableMapping.ps1
