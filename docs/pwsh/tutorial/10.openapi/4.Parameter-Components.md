...
- **Solution**: Put validation on the *component variable/parameter* using PowerShell attributes like `[ValidateSet]`, `[ValidateRange]`, `[ValidatePattern]`, or set constraints directly on `[OpenApiParameterComponent(...)]` (e.g., `Minimum`, `Maximum`, `Pattern`, etc.).
