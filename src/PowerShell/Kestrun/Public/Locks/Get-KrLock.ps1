...
.SYNOPSIS
    ...
    This lock can be used to synchronize access to shared resources across different parts of the application or even across different instances of the application.
.DESCRIPTION
    ...
    ensuring that only one thread or process can access the resource at a time when using the lock. The function uses a registry of locks to manage and retrieve lock objects based on their associated keys.
    This is particularly useful in scenarios where multiple threads or processes need to coordinate access to shared resources, such as files, shared state, or critical sections of code.
.PARAMETER Key
...
.EXAMPLE
    ...
    access to the resource identified by "MyResourceLock" across different parts of the application or across different instances of the application.
.NOTES
