...
Describe 'Example 2.1-Multiple-Content-Types' -Tag 'Tutorial' {
    ...
    It 'Text/Json/Xml/Yaml endpoints return expected formats' {
        ...
        Invoke-ExampleRequest -Uri "http://127.0.0.1:$p/hello" -ExpectStatus 200 -BodyContains 'Hello, World!' -ContentTypeContains 'text/plain'
        $jsonResp = Invoke-ExampleRequest -Uri "http://127.0.0.1:$p/hello-json" -ExpectStatus 200 -ContentTypeContains 'application/json' -ReturnRaw
        Assert-JsonFieldValue -Json $jsonResp.Content -Field 'message' -Expected 'Hello, World!'
        $xmlResp = Invoke-ExampleRequest -Uri "http://127.0.0.1:$p/hello-xml" -ExpectStatus 200 -ContentTypeContains 'application/xml' -ReturnRaw
        ($xmlResp.Content -match '<message>Hello, World!</message>') | Should -BeTrue
        $yamlResp = Invoke-ExampleRequest -Uri "http://127.0.0.1:$p/hello-yaml" -ExpectStatus 200 -ReturnRaw
