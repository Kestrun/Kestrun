...
Describe 'Example 16.4-Health-Process-Probe' -Tag 'Tutorial', 'Health' {
    BeforeAll {
        $script:instance = Start-ExampleScript -Name '16.4-Health-Process-Probe.ps1'
    }
    AfterAll {
        if ($script:instance) {
    ...
    It 'GET /healthz returns DotNetInfo probe' {
        $resp = Invoke-TestRequest -Uri "$( $script:instance.Url)/healthz" -UseBasicParsing -TimeoutSec 12 -Method Get -SkipHttpErrorCheck
