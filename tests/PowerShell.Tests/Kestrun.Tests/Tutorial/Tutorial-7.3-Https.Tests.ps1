...
Describe 'Example 7.3-Https' {
    ...
    It 'GET /secure returns hello on secondary listener (HTTPS)' {
        ...
        $resp = Invoke-TestRequest -Uri $uri -UseBasicParsing -TimeoutSec 5 -ErrorAction Stop -SkipCertificateCheck
        $resp.StatusCode | Should -Be 200
    ...
    It 'GET /secure fails on primary listener (HTTP)' {
        ...
        { Invoke-TestRequest -Uri $uri -UseBasicParsing -TimeoutSec 5 -ErrorAction Stop } | Should -Throw
    }

    It 'GET /unsecure fails on secondary listener (HTTPS)' {
        $uri = "https://127.0.0.1:$($script:instance.Port+443)/unsecure"
        { Invoke-TestRequest -Uri $uri -UseBasicParsing -TimeoutSec 5 -ErrorAction Stop -SkipCertificateCheck } | Should -Throw
    }
}
