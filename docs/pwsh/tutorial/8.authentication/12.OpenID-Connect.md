...
File: [`pwsh/tutorial/examples/8.12-OIDC.ps1`][8.12-OIDC.ps1]

```powershell
{% include examples/pwsh/8.12-OIDC.ps1 %}
```
...
6. Routes: Add public landing and a protected `/oidc/*` group using `Oidc.Policy`.
...
pwsh docs/_includes/examples/pwsh/8.12-OIDC.ps1
```

```powershell
# After login at Duende, fetch claims
Invoke-WebRequest https://localhost:5000/oidc/me -SkipCertificateCheck | Select -Expand Content
```
...
[8.12-OIDC.ps1]: /pwsh/tutorial/examples/8.12-OIDC.ps1
