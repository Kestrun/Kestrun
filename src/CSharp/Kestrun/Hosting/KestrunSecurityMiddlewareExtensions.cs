...
public static class KestrunSecurityMiddlewareExtensions
{
    ...
    /// <summary>
    /// Adds Forwarded Headers to the application using the specified configuration delegate.
    /// </summary>
    /// <param name="host">The KestrunHost instance to configure.</param>
    ...
    public static KestrunHost AddForwardedHeaders(this KestrunHost host, Action<ForwardedHeadersOptions> opts)
    {
        if (host.Logger.IsEnabled(LogEventLevel.Debug))
        {
            host.Logger.Debug("Adding forwarded headers with configuration");
        }

        // Register the forwarded headers service
        _ = host.AddService(services =>
            {
                _ = services.Configure<ForwardedHeadersOptions>(options => opts(options)); // Always pass a delegate
            });

        // Apply the middleware
        return host.Use(app => app.UseForwardedHeaders());
    }

    /// <summary>
    /// Adds Forwarded Headers to the application using the specified <see cref="ForwardedHeadersOptions"/>.
    /// </summary>
    /// <param name="host">The KestrunHost instance to configure.</param>
    ...
    public static KestrunHost AddForwardedHeaders(this KestrunHost host, ForwardedHeadersOptions opts)
    {
        if (host.Logger.IsEnabled(LogEventLevel.Debug))
        {
            host.Logger.Debug("Adding forwarded headers with configuration: {ForwardedHeadersOptions}", opts);
        }
        ...
        _ = host.AddService(services =>
        {
            ...
            {
                options.ForwardedHeaders = opts.ForwardedHeaders;
                options.ForwardLimit = opts.ForwardLimit;
                options.KnownNetworks.Clear();
                foreach (var network in opts.KnownNetworks)
                {
                    options.KnownNetworks.Add(network);
                }
                options.KnownProxies.Clear();
                foreach (var proxy in opts.KnownProxies)
                {
                ...
                options.RequireHeaderSymmetry = opts.RequireHeaderSymmetry;
                options.ForwardedForHeaderName = opts.ForwardedForHeaderName;
                options.ForwardedHostHeaderName = opts.ForwardedHostHeaderName;
                options.ForwardedProtoHeaderName = opts.ForwardedProtoHeaderName;
                options.ForwardedPrefixHeaderName = opts.ForwardedPrefixHeaderName;
                options.OriginalForHeaderName = opts.OriginalForHeaderName;
                options.OriginalHostHeaderName = opts.OriginalHostHeaderName;
                options.OriginalProtoHeaderName = opts.OriginalProtoHeaderName;
                options.OriginalPrefixHeaderName = opts.OriginalPrefixHeaderName;
            });
        });
