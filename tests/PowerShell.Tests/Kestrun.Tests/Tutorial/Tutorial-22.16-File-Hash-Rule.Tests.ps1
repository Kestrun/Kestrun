...
Describe 'Example 22.16 File hash upload (rules)' -Tag 'Tutorial', 'multipart/form', 'Slow' {
    BeforeAll {
        $script:instance = Start-ExampleScript -Name '22.16-File-Hash-Rule.ps1'
    }
    AfterAll {
        if ($script:instance) {
    ...
    It 'Uploads a binary file and returns hashes' {
        $testFile = Join-Path ([System.IO.Path]::GetTempPath()) 'kestrun-upload-hash-rule.bin'
