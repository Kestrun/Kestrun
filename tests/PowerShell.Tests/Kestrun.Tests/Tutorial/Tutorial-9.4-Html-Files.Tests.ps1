...
Describe 'Example 9.4-Html-Files' {
    BeforeAll { $script:instance = Start-ExampleScript -Name '9.4-Html-Files.ps1' }
    AfterAll {
        if ($script:instance) {
    ...
    It 'HTML routes behave as expected' {
        ...
        $page = Invoke-TestRequest -Uri "$($script:instance.Url)/page" -UseBasicParsing -TimeoutSec 8
    ...
    It 'Template routes behave as expected' {
        ...
        $inline = Invoke-TestRequest -Uri "$($script:instance.Url)/inline" -UseBasicParsing -TimeoutSec 8
    ...
    It 'Download routes behave as expected' {
        ...
        $dl = Invoke-TestRequest -Uri "$($script:instance.Url)/download" -UseBasicParsing -TimeoutSec 8
