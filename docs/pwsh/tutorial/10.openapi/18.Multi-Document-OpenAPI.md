...
10. **Expose OpenAPI routes**:
    ...
    - Call [Add-KrOpenApiRoute][Add-KrOpenApiRoute] again with `-DocumentId 'WebHookDoc'` to expose the second document. When a non-default document id is used with the default pattern, Kestrun serves it at `/openapi/{documentId}/{version}/openapi.{format}`.
